...
function buildSources({ wordpress, gsc, ga4, audit, serpRows, aiRows, gscDaily, ga4Daily, allowed, scope }) {
  ...
  return [
    ...
    source('Search Console', gsc?.created_at, 72, `${gscDaily.length} page-day rows · observed ${minDate(gscDaily) || '—'} to ${maxDate(gscDaily) || '—'}`, 'Measured'),
    source('Google Analytics', ga4?.created_at, 72, `${ga4Daily.length} page-day rows · observed ${minDate(ga4Daily) || '—'} to ${maxDate(ga4Daily) || '—'}`, 'Measured'),
